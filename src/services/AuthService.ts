...
const getCurrentUser = async (): Promise<IUserSuccess> => {
  ...
  const userId = res && res.data && res.data.id;
  const userRole = await HttpService.get<IObjectEnvelope<any>>(`/v1/users/${userId}/roles`);
  return {
    data: {
      ...res.data,
      permissions: new Set(res.data.permissions),
      role: userRole && userRole.data && userRole.data[0].name
    }
  };
};
