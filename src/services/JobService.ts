...
const getJobNameOfCreditType = (job: Partial<IJob> | ILinkedJob) => {
  return `#CN${job.id}${job.assigned_location ? `-${job.assigned_location.code}` : ''}`;
};
...
const getJobsNewInventory = async (
  locationId: number,
  searchString?: string | null,
  page?: number
): Promise<IJobsSuccess> => {
  const res = await HttpService.get<IJobsSuccess>('/v1/jobs/new-inventory', {
