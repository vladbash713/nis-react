...
class ContactDetailsPage extends ScrollToComponent<IProps & IConnectProps, IState> {
  ...
  private createOrUpdateContact = (data: Partial<IContact>) => {
    ...
    return promise
      .then((res: IContactSuccess) => ContactService.getContactById(res.data.id))
      .then((pres: any) => {
        const action = contactId ? updateContactInList(pres.data.id, pres.data) : addContactToList(pres.data);
