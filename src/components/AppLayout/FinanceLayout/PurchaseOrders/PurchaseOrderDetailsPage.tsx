...
class PurchaseOrderDetailsPage extends FinanceDetailsPage<IProps, IState> {
  ...
  private sendAdjustRequest = async (data: AdjustFormValues) => {
    PurchaseOrdersService.adjustPurchaseOrder(data.id, data.address, data.name).then(async () => {
  ...
  private getDropdownMenuItems = (context: IUserContext) => {
    ...
    const items: IDropdownMenuItem[] = [
      ...
      {
        name: 'Adjust Purchase Order To...',
        onClick: this.adjustPurchase,
        disabled: !this.editablePurchase()
      },
      {
        type: 'divider'
      },
        ...
        {context => {
          ...
          return (
            <>
              ...
              {this.editablePurchase() && showAdjustModal && (
                <ModalAdjustInvoice
                  isOpen={showAdjustModal}
                  onSubmit={this.sendAdjustRequest}
                  onClose={this.hideAdjustModal}
                  title="Edit Purchase Order To"
                  id={data.id}
                  initialValues={{
