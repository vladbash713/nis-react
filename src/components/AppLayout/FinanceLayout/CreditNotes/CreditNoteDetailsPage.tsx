...
class CreditNoteDetailsPage extends FinanceDetailsPage<IProps, IState> {
  ...
  private sendAdjustRequest = async (data: AdjustFormValues) => {
    CreditNotesService.adjustCreditNote(data.id, data.address, data.name).then(async () => {
  ...
  private getDropdownMenuItems = (context: IUserContext) => {
    ...
    const items: IDropdownMenuItem[] = [
      ...
      {
        name: 'Adjust Credit Note To...',
        onClick: this.adjustCreditNote,
        disabled: !this.editableCreditNote()
      },
      {
        type: 'divider'
      },
  ...
  private adjustCreditNote = () => {
    this.showAdjustModal();
  };

  private editableCreditNote = () => {
  ...
  public render() {
    ...
    const {showApproveModal, disableApproveButton, showAdjustModal} = this.state;
        ...
        {context => {
          ...
          return (
            <>
              ...
              {this.editableCreditNote() && showAdjustModal && (
                <ModalAdjustInvoice
                  isOpen={showAdjustModal}
                  onSubmit={this.sendAdjustRequest}
                  onClose={this.hideAdjustModal}
                  title="Edit Credit Note To"
                  id={data.id}
                  initialValues={{
