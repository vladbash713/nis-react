...
class FinanceFormUnwrapped extends React.PureComponent<IProps> {
  ...
  private onJobSelect = async (data: any) => {
    ...
    if (this.props.isNew) {
      ...
      if (this.props.isInvoice && data.insurer) {
        const termsDays = data.insurer.default_payment_terms_days || 1;
        const dueAt = moment(this.props.formData.date).add(termsDays, 'days');
        this.props.change('due_at', dueAt);
      }
    }
  ...
  private getContactName = (contact: any) => {
    ...
    const recipientName = formData && formData.recipient_name;
    return recipientName || contact.name || ContactService.getContactName(contact);
  };
