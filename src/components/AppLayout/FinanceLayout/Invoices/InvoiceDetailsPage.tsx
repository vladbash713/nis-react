...
import {IJobStatus, IJob} from 'src/models/IJob';
...
import StatusListItem, {StatusMessage, StatusDateLabel} from 'src/components/Layout/StatusListItem';
import ModalRequestApproval, {IFormValues} from 'src/components/Modal/Finance/ModalRequestApproval';
import ModalAdjustInvoice, {IFormValues as AdjustFormValues} from 'src/components/Modal/Finance/ModalAdjustInvoice';
...
class InvoiceDetailsPage extends FinanceDetailsPage<IProps, IState> {
  ...
  private getDropdownMenuItems = (context: IUserContext) => {
    ...
    const items: IDropdownMenuItem[] = [
      ...
      {
        name: 'Adjust Invoice To...',
        onClick: this.adjustInvoice,
        disabled: !this.editableInvoice()
      },
      {
        type: 'divider'
      },
  ...
  private sendAdjustRequest = async (data: AdjustFormValues) => {
    this.setState({pageLoading: true});
    InvoicesService.adjustInvoice(data.id, data.address, data.name).then(async () => {
      await this.fetchInvoice();
  ...
  private showAdjustModal = () => {
    this.setState({showAdjustModal: true});
  };

  private hideAdjustModal = () => {
  ...
  private showAddCreditModal = () => {
    ...
    const totalAmount = data && (data.total_amount as number);
    const amountDue = data && (data.amount_due as number);
    if (amountDue < totalAmount) {
      Notify(NotifyType.Danger, 'Unable to apply a credit note value that is higher than the invoice balance due');
    } else {
  ...
  private editableInvoice = () => {
    ...
    if (
      ...
      (role === 'administrator' ||
        role === 'Admin' ||
        role === 'branch_franchise_manager' ||
        role === 'cco' ||
        role === 'cso_am')
    ) {
      return true;
    }
    if (this.isApproved() && (role === 'administrator' || role === 'Admin')) {
      return true;
    }
    return false;
  };
  ...
  public render() {
    ...
    if (pageLoading || ((!financeEntity.ready || financeEntity.loading) && !this.isNew && !this.alreadyLoaded)) {
        ...
        {context => {
          ...
          return (
            <>
              ...
              {this.editableInvoice() && showAdjustModal && (
                <ModalAdjustInvoice
                  isOpen={showAdjustModal}
                  onSubmit={this.sendAdjustRequest}
                  onClose={this.hideAdjustModal}
                  title="Edit Invoice To"
                  id={data.id}
                  initialValues={{
                      ...
                      {data.payments && data.payments.length > 0 && (
                        <StatusMessage className="d-flex flex-row align-items-end">
                          <div className="d-flex flex-column align-self-center flex-grow-1">
                            ${data.payments[0].pivot.amount} {data.payments[0].type}{' '}
                            {(data.payments[0].type === 'credit_note' &&
                              JobService.getJobNameOfCreditType(data.job as Partial<IJob>)) ||
                              JobService.getJobName(data.job as Partial<IJob>)}{' '}
                            payment made by user {data.location && data.location.name}
                          </div>
                          <StatusDateLabel className="d-flex flex-column align-self-center">
                            {moment((data.payments && data.payments[0].paid_at!) || '').format(FRONTEND_DATE_TIME)}
                          </StatusDateLabel>
                        </StatusMessage>
                      )}
                    </div>
