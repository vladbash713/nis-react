...
class JobSiteSurvey extends React.PureComponent<IConnectProps & RouteComponentProps<IRouteParams>> {
  ...
  private submitSurveyForm = (data: {
    ...
    if (data.rooms) {
      if (data.edited) {
        promises.push(this.submitRooms(data.rooms, data.editingId));
      } else {
  ...
  private submitRooms = async (rooms: Array<{flooring_type: any; name: string}>, areaID?: number | string) => {
