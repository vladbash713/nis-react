...
class JobSiteSurveyFormForm extends React.PureComponent<
  ...
  IState
> {
  public state = {addNewRoom: false, edited: false, showModal: false, editingId: null};
  ...
  private onSaveRoom = (index: number) => (data: any) => {
    const {edited, editingId} = this.state;
    const savedRoom = data.rooms[index];
    const isNewRoom = savedRoom.name && savedRoom.flooring_type;
    if (isNewRoom) {
      this.setState({edited: false, editingId: null});
      return this.props.onSubmit({rooms: [savedRoom], edited, editingId});
    }
      ...
      ? this.props.jobRooms.map((room: IRoomType, index: number) => {
          ...
          return (
            <tr key={`room-tr-${index}`}>
              ...
              <td>{allowManage && this.renderRoomControls(onRemove, onCopy, onEditRoom, isRemovingNow)}</td>
            </tr>
          );
        })
      : null;
  ...
  private editExistRoom = (room: {name: string; flooring_type: any; areaId: number | string | null}) => {
    this.setState({edited: true, editingId: room.areaId});
    this.clippedRoom = room;
    this.addNewRoom();
  };

  private renderRoomControls = (
    onRemove: () => any,
    onCopy: (() => any) | null,
    onEditRoom: (() => any) | null,
    ...
    const editRoom = disabled ? () => null : onEditRoom ? onEditRoom : () => null;
    return (
      <IconWrapper disabled={disabled || this.props.disabled}>
        ...
        <div className="icon">
          <Icon name={IconName.FileEdit} onClick={editRoom} />
        </div>
        <div className="icon">
