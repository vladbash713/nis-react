...
class JobMenu extends React.PureComponent<RouteComponentProps<IParams> & IProps & IWithDataProps & IConnectProps> {
  ...
  private getRestMenuItems(context: IUserContext): IMenuItem[] {
    ...
    return [
      ...
      {
        name: 'Create PO...',
        disabled: !context.has(Permission.PURCHASE_ORDERS_MANAGE) || !jobHasInvoiceToContact,
        onClick: this.createPOFromJob
      },
      {
        name: 'Create CN...',
        disabled: !context.has(Permission.CREDIT_NOTES_MANAGE) || !jobHasInvoiceToContact,
  ...
  public render() {
    ...
    const isSetAssigned = job.assigned_location_id && job.assigned_location_id > 0 ? true : false;
    const isSetBelong = job.owner_location_id && job.owner_location_id > 0 ? true : false;
    return (
                ...
                <div>
                  ...
                  {context.has(Permission.USERS_VIEW) && (
                    <UsersControl
                      ...
                      disabled={
                        disabled || !context.has(Permission.JOBS_ASSIGN_STAFF) || !isSetAssigned || !isSetBelong
                      }
                    />
                  )}
