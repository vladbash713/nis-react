...
import {ConfigProps, Field, InjectedFormProps, reduxForm, formValueSelector} from 'redux-form';
...
class JobForm extends React.PureComponent<ICombinedProps> {
  ...
  private handleLocationChange = (location: any): void => {
    const {ownerLocation, assignedLocation, change, userLocations, locations} = this.props;
    if (!ownerLocation) {
      const foundLocation = locations.data && locations.data.data.find(item => item.id === location.id);
      if (foundLocation) {
        change('owner_location', foundLocation);
      }
    }
    if (!assignedLocation) {
      const foundLocation = userLocations && userLocations.find(item => item.id === location.id);
      if (foundLocation) {
        change('assigned_location', foundLocation);
      }
    }
  };
...
const mapStateToProps = (state: IAppState) => ({
  ...
  assignedLocation: formValueSelector(JobFormName)(state, 'assigned_location'),
  ownerLocation: formValueSelector(JobFormName)(state, 'owner_location')
});
