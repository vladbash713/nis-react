...
class JobNoteOrReply extends React.PureComponent<IProps, IState> {
  ...
  private renderMenu = (context: IUserContext): IMenuItem[] => {
    ...
    const {onDelete, allowEditNote, noteOrReply} = this.props;
    ...
    if (this.isReply) {
      ...
      if ((noteOrReply as IMessage).is_incoming) {
        result.push({
          name: 'Reply To All',
          onClick: () => this.props.onReply && this.props.onReply(noteOrReply as IMessage)
        });
      }
    }
