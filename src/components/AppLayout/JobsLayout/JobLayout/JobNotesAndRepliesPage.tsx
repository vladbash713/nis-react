...
import EmailReplyForm, {IFormData, EmailReplyTypes} from './EmailReplyForm';
import JobNoteOrReply from './JobNoteOrReply';
import {IMessage, IMessageRecipient, INote, MessageType} from 'src/models/INotesAndMessages';
...
class JobNotesAndRepliesPage extends ScrollToComponent<RouteComponentProps<IParams> & IConnectProps, IState> {
  public state = {
    replyMessage: null
  };
  ...
  private get emailReplyFormInitialValues() {
    const {job} = this.props;
    const initialValues = Object();
    if (job.data && job.data.id) {
      initialValues.subject = `Job No. #${job.data.id}`;
      const replyMessage = this.state.replyMessage as IMessage | null;
      if (replyMessage) {
        initialValues.type = EmailReplyTypes.find(item => item.value === MessageType.EMAIL);
        initialValues.to = replyMessage.from_address;
        initialValues.cc = replyMessage.recipients
          .filter((recipient: IMessageRecipient) => recipient.address)
          .map((recipient: IMessageRecipient) => recipient.address)
          .join(', ');
      }
    }
  ...
  private onReplyMessage = (replyMessage: IMessage) => {
                ...
                {this.isEmailReplyVisible && !editForbidden && showReplyForm && (
                  <NoteComponentWrapper padding="40px 30px 70px 30px">
                    <EmailReplyForm
                      ...
                      initialValues={this.emailReplyFormInitialValues}
                    />
                  </NoteComponentWrapper>
                )}
                {notesAndReplies && (
