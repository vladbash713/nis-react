...
class ModalFilmstrip extends React.PureComponent<IModal & IProps & IConnectProps, IState> {
  ...
  private downloadPhoto = () => {
    const {inventoryItem, photos} = this.props;
    ...
    PhotoService.downloadPhoto(
      photo.id,
      `${inventoryItem.id}_${photo.description ? photo.description.split(' ').join('_') : ''}_${moment(
        photo.updated_at
      ).format('YYYYMMDD')}.${photo.mime_type.split('/')[1]}`
    )
      .then(() => {
  ...
  private deletePhoto = async () => {
    // const {photos, inventoryItem, updateParentPhotos, dispatch} = this.props;
    // const photo = photos[this.state.currentIndex];
    const {updateParentPhotos, dispatch} = this.props;

    const res = await dispatch(openModal('Confirm', 'Do you really want to delete this photo from this item?'));

    if (res) {
      this.setState({bottomToolsLoading: true});

      try {
        // await InventoriesService.detachPhoto(inventoryItem.id, photo.id);
  ...
  public render() {
    const {isOpen, onClose, customModal, inventoryItem, photos, noUpdate, onlyView} = this.props;
