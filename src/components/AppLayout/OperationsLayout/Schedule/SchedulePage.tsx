...
class SchedulePage extends React.PureComponent<IProps, IState> {
  ...
  private loadRuns = async () => {
    ...
    if (date && location) {
      await dispatch(
        searchRuns({
          date: ((date as unknown) as Moment).format(BACKEND_DATE_TIME),
          location_id: ((location as unknown) as ILocation).id
        })
      );
      const {runs} = this.props;
      if (!runs.loading && runs && runs.data && runs.data.length === 0) {
        this.addNewRun();
      }
    }
  };
    ...
    if (location) {
      await TaskService.addTaskToRun(+runId, task.id, {
        starts_at: DateTransformer.dehydrateDateTime(
          (data.starts_at as Moment).add(-((location as unknown) as ILocation).tz_offset, 'm')
        ) as string,
        ends_at: DateTransformer.dehydrateDateTime(
          (data.ends_at as Moment).add(-((location as unknown) as ILocation).tz_offset, 'm')
        ) as string
      });
    }
