...
class OperationsJobsTabComponent extends React.PureComponent<IProps & IWithDataProps, IState> {
  ...
  private sortTasks = (tasks: ITask[]) => {
    ...
    if (sortType === SortTypes.Name) {
      toSchedule = orderBy(toSchedule, [(task: ITask) => task.name], ['asc']);
      scheduled = orderBy(scheduled, [(task: ITask) => task.name], ['asc']);
    }
  ...
  private sortItems = (): IMenuItem[] => {
    ...
    return [
      ...
      {
        name: SortTypeNames[SortTypes.Name],
        classNames: sortType === SortTypes.Name ? 'active' : '',
        onClick: () => this.setState({sortType: SortTypes.Name})
      },
      {
