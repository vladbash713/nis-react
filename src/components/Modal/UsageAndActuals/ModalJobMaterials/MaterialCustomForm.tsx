...
> {
  ...
  public componentDidMount() {
    ...
    if (this.props.initialValues && this.props.initialValues.default_buy_cost_per_unit) {
      change('default_buy_cost_per_unit', formatPrice(this.props.initialValues.default_buy_cost_per_unit as number));
    }
    if (this.props.initialValues && this.props.initialValues.default_sell_cost_per_unit) {
      change('default_sell_cost_per_unit', formatPrice(this.props.initialValues.default_sell_cost_per_unit as number));
    }
  }
  ...
  private setTotalAmount = () => {
    setTimeout(() => {
      ...
      let sellCostValue: number | string = 0;
      if (sellCost) {
        sellCostValue = (sellCost as string).split('$')[1];
        if (!sellCostValue) {
          sellCostValue = parseInt(sellCost.toString(), 10);
        } else {
          sellCostValue = parseInt(sellCostValue, 10);
        }
      }
      change('total_amount', formatPrice((sellCostValue > 0 ? sellCostValue : 0) * (quantity > 0 ? quantity : 0)));
    });
  };

  private onMaterialChange = (data: any) => {
    const {change} = this.props;
    change('default_buy_cost_per_unit', formatPrice(data.default_buy_cost_per_unit));
    change('default_sell_cost_per_unit', formatPrice(data.default_sell_cost_per_unit));
  };
