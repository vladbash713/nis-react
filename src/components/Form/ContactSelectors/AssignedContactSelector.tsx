...
class AssignedContactSelector extends React.PureComponent<IProps> {
  ...
  private includeInContact(value?: string) {
    if (!this.props.contactValue || !value) {
      return false;
    }

    if (!!this.props.multiple) {
      const contactValue = this.props.contactValue
        .split(',')
        .map(v => v.trim())
        .pop();
      return !!contactValue && value.toLowerCase().includes(contactValue.toLowerCase());
    }

    return value.toLowerCase().includes(this.props.contactValue.toLowerCase());
  }
